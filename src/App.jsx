...
export default function RoomieTaskApp() {
  ...
  const sendLineNotify = async (type, data) => {
    let text = '';
    if (type === 'RELEASE') text = `🚨 求救！\n\n${data.user} 無法執行「${data.task}」\n💰 釋出賞金：$${data.price}\n👉 有空的人請救援！`;
    else if (type === 'CLAIM') text = `🦸‍♂️ 救援成功！\n\n${data.user} 接手了「${data.task}」\n💵 獲得賞金 $${data.price}`;
    else if (type === 'COMPLETE') text = `✅ 任務完成\n\n${data.user} 已完成「${data.task}」`;
  ...
  const confirmDeleteUser = (userId) => {
    ...
    showConfirm('刪除室友', `確定要刪除 ${userToDelete.name} 嗎？`, () => {
      ...
      setCurrentCycleTasks(prev => prev.map(t => t.currentHolderId === userId ? { ...t, status: 'open', currentHolderId: null } : t));
      closeConfirmModal();
    });
  };

  const executeSettlement = (fromId, toId, amount) => {
    showConfirm('確認還款', `確定 ${getUserName(fromId)} 已經支付 $${amount} 給 ${getUserName(toId)} 了嗎？`, () => {
  ...
  const LoadMoreButton = ({ onClick }) => (
    <div className="p-2 text-center border-t border-gray-50">
      <button onClick={onClick} className="text-xs text-[#28C8C8] hover:text-[#20a0a0] font-medium flex items-center justify-center gap-1 w-full py-2 hover:bg-[#28C8C8]/5 rounded transition-colors">
        ...
        {view === 'roster' && (
          <div className="space-y-4">
            ...
            <div className="flex bg-gray-100 p-1 rounded-xl">
              <button onClick={() => setRosterViewMode('list')} className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-bold transition-all ${rosterViewMode === 'list' ? 'bg-white text-[#28C8C8] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                <List size={16} /> 清單模式
              </button>
              <button onClick={() => setRosterViewMode('calendar')} className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-bold transition-all ${rosterViewMode === 'calendar' ? 'bg-white text-[#28C8C8] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                            ...
                            {displayedAllTasks.map(task => {
                              ...
                              return (
                                <div key={task.id} className={`p-4 flex items-center justify-between transition-colors ${isOpen ? 'bg-red-50/50' : 'hover:bg-gray-50'}`}>
                                      ...
                                      <div className="flex items-center gap-2 mt-1">
                                        ...
                                        {!isDone && (
                                          <div className="flex items-center gap-1.5">
                                            {isOpen ? <span className="text-xs text-red-500 font-medium">賞金 ${task.price}</span> : (
                                              <><div className={`w-3 h-3 rounded-full ${getUserAvatar(task.currentHolderId)}`}></div><span className={`text-xs ${isMine ? 'font-bold text-[#28C8C8]' : 'text-gray-500'}`}>{getUserName(task.currentHolderId)}{isMine && ' (我)'}</span></>
                                            )}
                                          </div>
                                        )}
                                      </div>
                                  ...
                                  <div>
                                    {isOpen ? (
                                       <button onClick={() => claimBounty(task.id)} className="bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm shadow-red-200 active:scale-95 transition-transform">接單 +${task.price}</button>
                                    ) : isDone ? (
                                       <CheckCircle2 className="text-green-300" size={24} />
                                    ) : isMine ? (
                                       <button onClick={() => completeTask(task.id)} disabled={isTaskFuture} className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-colors ${isTaskFuture ? 'border-gray-200 text-gray-300 cursor-not-allowed bg-gray-50' : 'border-[#28C8C8]/30 hover:bg-[#28C8C8]/10 text-[#28C8C8]'}`}><CheckCircle2 size={18} /></button>
                                    ) : (
            ...
            <div className="mt-6 flex justify-center pb-20">
               <button onClick={() => dispatchTasksFromConfig(true)} className="text-xs text-gray-400 hover:text-[#28C8C8] flex items-center gap-1">
          ...
          <div className="animate-fade-in">
             <div className="bg-gradient-to-br from-[#28C8C8] to-[#1facac] rounded-2xl p-6 text-white shadow-xl mb-6">
               <div className="flex justify-between items-start">
                 <div><p className="text-white/80 text-sm mb-1">我的本月收支</p><h2 className={`text-4xl font-bold font-mono text-white`}>{currentUser.balance > 0 ? '+' : ''}{currentUser.balance}</h2></div>
        ...
        {view === 'settings_editor' && (
          <div className="bg-white flex flex-col h-full animate-slide-up">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-2"><User size={16} /> 起始負責人 (誰先開始)</label>
                <div className="relative">
