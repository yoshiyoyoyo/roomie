...
import { 
  ...
  ArrowRight, AlertCircle, Sparkles, ChevronRight
} from 'lucide-react';
...
export default function RoomieTaskApp() {
  ...
  // 列表控制
  const [myTasksLimit, setMyTasksLimit] = useState(3);
  const [allTasksLimit, setAllTasksLimit] = useState(3);
  ...
  const enterGroup = async (gId, user) => {
    ...
    onValue(ref(db, `groups/${gId}`), (snap) => {
      const data = snap.val();
      if (data) {
        // 使用 Object.values 並根據 joinedAt 排序使用者，確保輪班順序一致
        const userList = data.users ? Object.values(data.users).sort((a, b) => (a.joinedAt || 0) - (b.joinedAt || 0)) : [];
        setUsers(userList);
  ...
  const checkAndGenerateTasks = async (gId, data) => {
    ...
    // 取得排序後的 User IDs (確保 A->B->C->D 順序穩定)
    const userList = Object.values(data.users).sort((a, b) => (a.joinedAt || 0) - (b.joinedAt || 0));
    ...
    configs.forEach(cfg => {
      ...
      while (nextDate <= limitDate) {
        ...
        // --- 決定負責人 (Round Robin) ---
        let targetId = cfg.nextAssigneeId; // 優先使用系統紀錄的「下一位」

        // 如果沒有紀錄下一位 (第一次或重置)，則使用當前設定的 nextAssigneeId，或列表第一位
        if (!targetId) targetId = userIds[0];
        
        // 防呆：如果該負責人已離開群組，則重置為第一位
        if (!userIds.includes(targetId)) targetId = userIds[0];
        ...
        // 2. 計算「再下一位」是誰，並寫入 Config
        const currentIndex = userIds.indexOf(targetId);
  ...
  // 清除某個 Config 衍生的所有未來 Pending 任務 (用於修改或刪除規則時)
  const clearFutureTasks = async (configId) => {
    const tasksToRemove = currentCycleTasks.filter(t => t.configId === configId && t.status === 'pending');
    const updates = {};
  ...
  const saveConfig = async () => {
    ...
    // 如果是編輯，先清除舊的未來排程，讓系統重新根據新規則排
    if (editingConfigId) {
      await clearFutureTasks(id);
    }

    const configData = { ...configForm, id, freq: freqStr, defaultAssigneeId: assignee, nextAssigneeId: assignee }; // 重置輪班起點為設定者
  ...
  const allTasks = visibleTasks; // 顯示所有人
          ...
          <div className="space-y-6">
            <section>
              <div className="sticky top-0 bg-gray-50 z-20 py-2 flex justify-between items-center mb-2" onClick={() => setIsMyTasksCollapsed(!isMyTasksCollapsed)}>
                <h3 className="font-bold text-gray-800 flex items-center gap-2 text-lg"><CheckCircle2 size={20} className="text-[#28C8C8]"/> 近期待辦 (45天)</h3>
            ...
            <section>
              <div className="sticky top-0 bg-gray-50 z-20 py-2 flex justify-between items-center mb-2" onClick={() => setIsTaskListCollapsed(!isTaskListCollapsed)}>
                <h3 className="font-bold text-gray-800 flex items-center gap-2 text-lg"><Users size={20}/> 任務列表 (45天)</h3>
          ...
          <div className="bg-white rounded-t-3xl sm:rounded-3xl p-6 space-y-5 animate-in slide-in-from-bottom-5">
            ...
            <div className="grid grid-cols-2 gap-5">
              ...
              <div className="space-y-1">
                ...
                <select value={configForm.defaultAssigneeId} onChange={e => setConfigForm({...configForm, defaultAssigneeId:e.target.value})} className="w-full h-14 px-3 bg-gray-50 rounded-2xl font-bold outline-none appearance-none text-lg">
